"""Altere o programa anterior e permita que o usuário salve o resultado da pesquisa em um  arquivo  no  formato  XML.(obs.:  faça  o  download  do  arquivo  xml  e  abra-o  na  sua máquina, utilizando o browser, para ver se ele está correto.)"""
lista=[]
lista2=[]
resp="S"
n=0
...
if resp=="S":
    for cont in range(esc):
        if lista[cont]<=13:    
            if lista2[cont]<altm:
                n=n+1                
resp2=input("voçê quer salvar o resultado em um arquivo .Xml? S-Sim N-Não ").upper()
while resp2!="S" and resp2!="N":
    resp2=input("por favor digite S-Sim ou N-Não ").upper()
if resp2=="S":    
    arquivo = open('1.Xml','w')
